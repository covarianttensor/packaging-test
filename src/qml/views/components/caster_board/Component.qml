...
Flickable {
    ...
    Keys.onPressed: {
        onKeyPressedEvent("a caster player", event.key)
    }
    ...
    /*Keys.onPressed: {
        if (event.key === Qt.Key_A) {
            for (var i = 0; i < playerRepeater.count; i++) {
                var player = playerRepeater.itemAt(i);
                console.log("Player " + i + " property: " + player.volume);
                // Access the desired property of each player
